...
test('published cards render metadata and exact app actions for every route', { timeout: 30000 }, async () => {
  ...
  try {
    ...
    for (const host of ['getrec.me', 'www.getrec.me', 'astirmovement.com']) {
      const response = await requestWithHost(`${base}/support?from=old-link`, host);
      const shouldRedirect = cutoverEnabled && host !== 'astirmovement.com';
    ...
    for (const [root, identifier] of Object.entries(paths)) {
      const response = await requestWithHost(`${base}/cards/${root}/${identifier}?card=${'a'.repeat(48)}`, 'getrec.me', { 'User-Agent': 'facebookexternalhit/1.1' });
