...
globalThis.fetch = async (input, options) => {
  if (String(input) === 'https://share-fixture.supabase.co/rest/v1/rpc/share_card_preview') {
    ...
    const generic = body.input_token === 'c'.repeat(48);
    const valid = (body.input_token === 'a'.repeat(48) || generic) && identifiers[body.input_kind] === body.input_identifier;
    return new Response(JSON.stringify(valid ? generic ? { preview_mode: 'generic' } : {
